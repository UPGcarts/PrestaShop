...
class UpgPrestashopCallback implements \Upg\Library\Callback\ProcessorInterface
{
    ...
    public function __construct(Hostedpayments $module, \Upg\Library\Config $config)
    {
        $this->module = $module;
        $this->config = $config;
    }
     ...
     * @param $merchantID This is the merchantID assigned by Gateway.
     * @param $storeID This is the store ID of a merchant assigned by Gateway as a merchant can have more than one store.
    ...
    public function run()
    {
        if($this->notificationType == self::NOTIFICATION_TYPE_PAYMENT_STATUS && $this->resultCode == 0) {
            ...
            if($transactionId){
                $orderStatus = Configuration::get(Hostedpayments::UPG_STATUS_RETURNED);
                ...
                Db::getInstance()->update(
                    'upg_transaction',
                    array('paymentMethod' => pSQL($this->paymentMethod)),
                    'transaction_reference = "'.pSQL($transactionId).'"'
                );

                return Context::getContext()->link->getModuleLink('hostedpayments', 'success', array('cart_id'=>$cartId, 'transaction_id'=>$transactionId));
            }
        ...
        if(self::validateCallbackUrl($this->paymentInstrumentsPageUrl)) {
            return Context::getContext()->link->getModuleLink('hostedpayments', 'recover', array('url'=>$this->paymentInstrumentsPageUrl), true);
        }

        return Context::getContext()->link->getModuleLink('hostedpayments', 'error');
    }
