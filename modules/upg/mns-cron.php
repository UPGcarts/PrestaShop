...
include_once(_PS_MODULE_DIR_ . 'payco/Model/MnsProcess.php');
...
if ($results = Db::getInstance()->ExecuteS($sql)) {
    foreach ($results as $mns) {
        ...
        if($orderId) {
            ...
            if(!$processed) {
                Logger::addLog(
                    'Payco - MNS can no valid order state or transaction status was sent'.
            ...
            Logger::addLog(
                'Payco - MNS can not be processed as order can not be found'.' '.$mns['orderID'].' ',
                1,
                null,
                null,
                null, true
